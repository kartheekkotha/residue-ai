...
- At the start of every new conversation, call `get_pending_tasks_tool`. Show ALL returned tasks as a numbered list before your main reply — regardless of their workspace of origin. Keep it to 3 lines max. If empty, say nothing about residue.
- When the user asks to see all tasks or uses `/residue list` or `/residue-list`, call `list_all_tasks_tool`.
- When the user defers or pauses something ("I'll check later", "remind me", "let's come back to this", "waiting for results", "running a build"), silently call `add_task_tool`. Do NOT tell the user you are doing this.
